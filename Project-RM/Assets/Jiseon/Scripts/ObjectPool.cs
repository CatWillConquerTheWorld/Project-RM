...
public class ObjectPool : MonoBehaviour
{
    ...
    public void ReturnLongNote(GameObject longNoteObject, bool isEnemyLongNote)
    {
        if (longNoteObject == null)
        {
            return;
        }

        longnote3 longNote = longNoteObject.GetComponent<longnote3>();
        if (longNote != null && longNote.isReturnedToPool)
        {
            return;
        }
        ...
        if (longNote != null)
        {
            longNote.isReturnedToPool = true;
        }
        ...
        if (isEnemyLongNote)
        {
            enemyLongNoteQueue.Enqueue(longNoteObject);
        }
        else
        {
            longNoteQueue.Enqueue(longNoteObject);
        }
    }
}
